...
The default invocation carries no revision at all. `scripts/rebuild.sh` reads the canonical `refs/heads/main` tip itself with `git ls-remote` against the canonical remote — which fetches no objects and moves no ref in the checkout — and posts that exact commit, so no SHA is ever retyped between a note and a deploy: a mangled tail deployed a revision that never existed for six hours (#1032, #1033). It prints the resolved SHA before requesting admission.

`scripts/rebuild.sh` always names its target as `revision`, and `POST /api/runtime/deployments` accepts a `revision` only as a full 40-character commit SHA, so a full lowercase SHA is the one argument the script takes and the no-argument default resolves the tip to one before sending. Nothing that reads as a started deployment — `deployment key: …`, `deployment admitted: …` — is printed until the endpoint has accepted the request; a refused request prints its error on stderr and exits non-zero (#1309).
...
A Dependabot pull request opened before this workflow merged still carries older copies of the three gate workflows on its own branch, and a dispatch reads the workflow file from the branch it names, so those dispatches are refused there and the refresh fails. That is deliberate. The lockfile commit stands, and its head is a commit no gate has read; GitHub starts no workflow run of any kind for a push made with that token, so a refresh that reported success there would leave a head whose required checks never started. Comment `@dependabot rebase` before the lockfile commit lands to pull the current workflows onto the branch — Dependabot stops rebasing a pull request once anything else has pushed to it. If the commit has already landed, close and reopen the pull request: `reopened` reaches all three gates on the head as it stands, and the refresh itself finds `bun.lock` already resolved and dispatches nothing.
