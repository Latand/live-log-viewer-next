...
- `task_started` and `turn_started` clear `openTools` and `anonymousTools`. A new turn cannot inherit an earlier turn's unanswered calls; whatever was open belonged to a turn that will never complete them. `user_message` keeps its current behaviour (it opens a turn but a steer mid-turn must not drop the turn's own tools).
