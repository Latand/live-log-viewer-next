...
## Separate operator-worktime architecture (issue #763)

Issue #473 remains the authority for remote Viewer activity analytics. Its
bulk-heartbeat scheduler, `agent-log-viewer/...` entity namespace, agent turn
windows, local outbox, retry behavior, and delivery contract stay intact.

Issue #763 adds a separate local subsystem under `src/lib/worktime/`:

```text
human ingress
  -> stable operator provenance
  -> privacy-minimized canonical ledger
  -> complete-inventory historical reconciliation

raw WakaTime heartbeats
  -> exclude agent-log-viewer entities and boundary project
  -> real editor intervals

ledger events + editor intervals
  -> Europe/Kyiv daily calculator
  -> model-free local export
  -> receipt-backed private-draft seam
```

The worktime subsystem never writes a projection into the #473 entity
namespace. It treats every entity in that namespace as synthetic input and
reports its excluded duration for audit.

### Operator-event identity

`OperatorEventProvenance` carries `{ id, origin, relation }`. IDs accept a
bounded provider or client identifier without a product-specific prefix.
`origin` is `composer`, `realtime`, or `api-human`; `relation` is `direct` or
`copy`. Runtime commands validate the value, held deliveries include it in
their immutable request identity, and Codex structured-user markers persist it
for later complete scans. Claude delivery retains it in the Viewer-owned
delivery ledger. The worktime state stores only SHA-256 forms.

Direct provenance establishes operator authorship. Copy provenance cannot
establish it. Every occurrence joins on the source digest, moves the canonical
timestamp to the earliest occurrence, and accumulates ranked project evidence.
Multiple projects at the highest rank set the event to ambiguous regardless of
discovery order.

Historical human records without a stable ID use:

```text
sha256(normalized content) + sha256(canonical lineage root) + timestamp within 30 seconds
```

The fallback requires explicit typed/human evidence. Protocol, SDK, scheduler,
spawn, tool, sidechain, and compact-summary rows never enter the fallback.

### Inventory and persistence ordering

The sidecar walks a dedicated uncapped inventory of every configured
Claude/Codex account root. Sidebar project and card limits never constrain this
inventory. Registry generations provide durable project authority; a complete
transcript parse supplies cwd evidence for legacy artifacts. A day is eligible
only when the global walk is complete and every transcript file remains the
same size and mtime through its full parse. Any uncertainty returns an empty
incomplete result.

Ordering is strict:

1. Prove the complete inventory and derive privacy-minimized occurrences in memory.
2. Read the WakaTime credential and fetch raw editor heartbeats.
3. Open one state transaction, re-read the latest operator ledger, import occurrences, and calculate the missing day.

Steps 2 and 3 never run after an incomplete scan. This prevents partial
classification from retiring, rewriting, or publishing durable state. Live
ingress uses the same transaction helper, so a network-bound catch-up pass
cannot overwrite an operator event arriving concurrently.

### Daily calculator

`calculator.ts` owns Europe/Kyiv day boundaries, including 23-hour and 25-hour
DST days. It builds fixed operator episodes from deduplicated event timestamps,
unions them with per-project editor intervals, and resolves overlap with a
sorted boundary sweep. The configured priority list is highest-first. Unlisted
projects share rank zero; simultaneous winners enter ambiguity.

The persisted daily result includes canonical assigned intervals. Recalculation
therefore rebuilds boundaries from source evidence and cannot retain an old
singleton endpoint after a late earlier occurrence or overlap arrives.
Undelivered rollups are invalidated by changed canonical evidence. Delivered
rollups remain immutable for message idempotency.

### Export and delivery boundary

`GET /api/worktime?day=YYYY-MM-DD` is a loopback, operator-only model-free
export. It returns no transcript or account data. Lifecycle fields are separate
from calculation and use receipt-backed completion semantics.

`deliverPrivateDraft()` is the transport port. A successful send still leaves
the day pending until read-back confirms the same receipt, private destination,
and payload digest. The repository has no approved destination adapter at this
boundary. The core scheduler leaves `delivered_at` empty and exposes the draft
for a later owner of that external dependency.
