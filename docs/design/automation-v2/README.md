...
This design was verified against repository commit `d79b463e` (the tip of `main` on 2026-09-02). Every `file:line` below refers to that commit; the lane writes only under `docs/design/automation-v2/`, so the `src/` tree at every lane head is byte-identical to it and every citation holds at both. Usage figures were read from the operator's state store on 2026-09-02 and cover records created between 2026-08-19 and 2026-09-02.
...
Revision 3 (2026-09-02) answers review round 3: every mutation that creates or stops an attempt is revision-fenced and is refused while any attempt on any stage is unsettled, so two seats racing on a stale read yield one attempt (§3.3, ADR-0002 rule 2); the provider-throttle wait names its transcript evidence, its text classes and its ceiling (§3.4, ADR-0003); and migration seeds no checkpoints because attempts carry no commit SHA, with `"last-passed"` resolved from `lastPassedCommit` (§5).

Revision 4 (2026-09-02) answers review round 4, which reviewed lane head `7267f6f2` and returned five findings. Per finding: (1) applied: the `paused` flag still lands in slice 5, and the public read every transport shares derives the legacy `state: "paused"` and `pausedState` pair from it until slice 12, so every reader outside the engine keeps its contract; the two readers no lane owned (`src/lib/monitor/evidence.ts`, `src/components/tasks/taskReadiness.ts`) join lane G and slice 12 lists every reader that moves (§3.6, §6.1, §6.3). (2) Applied: `rerun-stage { stopCurrent: true }` is the explicit, attributed, fenced stop of the unsettled attempt on any stage, through the same identity-fenced stop path as `edit-stage { restart: true }`; without it, `rerun-stage` in `running` is refused naming that attempt, since a running record always has one (§3.3, ADR-0002 rule 2). (3) Applied: definition and graph edits (`edit-stage`, `add-stage`, `remove-stage`, `reorder-stage`, `set-edge`) and `note` are admitted on a `completed` record, which today is refused as terminal, so the next round's reviewer is bound before `rerun-stage` reopens the record and the binding's tick request binds the new attempt (§3.3, §4.1, ADR-0002). Inline edits on `rerun-stage` were the alternative and were not taken: they would duplicate `edit-stage`'s payload. (4) Applied: every mutation is stamped with the revision it produced; the fence (refusal on mismatch) is required for every action whose correctness depends on what the caller last read, now including `note`; `pause`, `resume`, `close` and the bookkeeping actions take it optionally because a stop-and-look action must never be refused for staleness, and the text says so (§3.3, ADR-0002 rule 2). (5) Applied: automatic `stage:<id>:<n>` checkpoints are cut; a pass already records its head on the attempt (`headAtSettle`, §3.5) and on `lastPassedCommit`, and `checkpoints` holds only what the `checkpoint` action names (§3.1, §3.8, §5, §7, §8).

## Decision in one paragraph

Keep the pipeline record as the one durable automation record and make it mutable after start. Retire the flow record, the workflow record and the `review-loop` stage kind; the only loop is a reviewer run stage with a fail edge back to a fixer stage, which is already how 275 of the last 293 pipelines were built. Add to the record a revision counter, an append-only attributed mutation log, a paused flag, checkpoints, per-attempt definition snapshots and a per-attempt `waiting` reason. Extend `pipeline_action` with `edit-stage`, `rerun-stage`, `checkpoint`, `note` and `answer`, lift the after-start refusals from `add-stage`, `remove-stage`, `reorder-stage` and `set-edge`, and fence every mutation that changes a definition, attaches a note, creates an attempt or stops one on `expectedRevision`. A mutation that lands on a stage with a running attempt is recorded and applied to the next attempt; it is never applied under the running one silently, and a caller who wants it now says `restart: true`. Replace "the first ended turn settles the stage" with "an ended turn settles the stage only when nothing is pending", where pending means a background task, a provider throttle, an account limit or a host between publications, each bounded by a ceiling. The MCP tool schema publishes every mutation with a typed payload; the Viewer UI later calls the same PATCH route with the same payloads.
...
type Pipeline = /* existing fields */ & {
  ...
  /** Replaces state: "paused" and pausedState on the durable record. A parked
      or running record stays parked or running while paused; the tick simply
      does not act. Until slice 12 the public read derives the legacy pair from
      this flag for readers outside the engine (§3.6). */
  paused: { at: string; actor: MutationActor } | null;
  /** Worktree states named by the `checkpoint` action, and only those: a pass
      already records its head on the attempt (headAtSettle, §3.5) and on
      lastPassedCommit, so nothing is checkpointed automatically. */
...
type PipelineCheckpoint = {
  name: string;          // free text given by the `checkpoint` action
  sha: string;
  at: string;
  actor: MutationActor;
  stageId: string;       // cursor stage and attempt when it was taken
  attempt: number;
};
```
...
type PipelineStageAttempt = /* existing fields */ & {
  ...
  waiting:
    ...
    | { kind: "provider-throttle"; since: string; until: string; resetKnown: boolean; continuations: number }
...
All mutations go through `pipeline_action` and `PATCH /api/pipelines/<id>` with the same payloads; `PIPELINE_ACTIONS` (`types.ts:323-341`) stays the single declaration. Each mutation is validated, applied inside the existing record transaction (`store.ts:729-736`), appended to `mutations` with its actor, and bumps `revision`. Every mutation is stamped with the revision it produced (`PipelineMutation.revision`), so every entry in the log is revision-attributed. The fence, a refusal on mismatch, is required for every action whose correctness depends on what the caller last read: a definition or graph edit, an action that creates or stops an attempt, and `note`, whose text addresses the stage and attempt history its author saw. A mismatch is refused with the current revision and the mutations since the caller's revision, so an agent re-reads and retries. `pause`, `resume`, `close` and the unchanged bookkeeping actions take `expectedRevision` optionally, and honour it when present, because they are how a caller stops something it may not have finished reading: refusing a pause or a close for staleness would leave an unwanted attempt running while the caller re-reads, and neither changes a definition, so the fence would protect nothing. That is the one deliberate exception to "every mutation is revision-fenced" in #1446, and it is the same exception the cap rule below makes for pause.

One predicate is evaluated inside the record transaction, after the fence. An attempt is **unsettled** while its state is `pending`, `spawning`, `running`, `reviewing` or `committing`, or its `waiting` field is set. A mutation that would create an attempt or commit the worktree (`rerun-stage`, `answer`, `skip-stage`, `checkpoint`) is refused while any attempt on any stage is unsettled, and the refusal names that attempt. An unsettled attempt is stopped only by an explicit, attributed, fenced request, never as a side effect of another action: `edit-stage { restart: true }` on its own stage, or `rerun-stage { stopCurrent: true }` when the re-run targets any stage. Both go through the same identity-fenced stop path (`stopPipelineStageAgent`, `engine.ts:614`). Without `stopCurrent`, a `rerun-stage` on a `running` record is always refused, because a running record always has an unsettled attempt: `pending` counts, and the attempt is created in the transaction that moves the cursor (below), so there is no gap in which the record is running with nothing to name. Each of these actions creates the attempt record itself (state `pending`, `definition: null`) in the transaction that moves the cursor, where today the tick creates it (`engine.ts:1875-1876`) after the action has set the cursor to `pending` (`engine.ts:4254`); that is what gives the predicate a record to read in the window before the next tick. The fence is what makes the predicate sufficient. A `rerun-stage` that creates attempt n+1 bumps `revision` in the same transaction, so a second seat holding the old revision is refused by the fence before the predicate is read, and a second seat that re-read sees attempt n+1 `pending` and is refused by the predicate. Neither path can create n+2, and the worktree has one owner.

| Action | Payload | Allowed when | Effect on a stage whose attempt is running | Fence |
| --- | --- | --- | --- | --- |
| `edit-stage` (replaces `override-stage`) | `stageId`, any of `prompt`, `role`, `engine`, `model`, `effort`, `access`, `sandbox`, `outputs`, `account`, plus `restart?: boolean` | any state except `closed`, any stage, any attempt history; `completed` is admitted so the next round's reviewer is bound before `rerun-stage` reopens the record (today `override-stage` refuses both terminal states, `engine.ts:905`, `4281`) | recorded as `pending-next-attempt`; with `restart: true` the running attempt is stopped through the identity-fenced stop path (`engine.ts:614`) and attempt n+1 starts from the current worktree | `expectedRevision` required |
| `add-stage` | `stage`, `index?` | any state except `closed` | inserted at its seam as today (`engine.ts:4022-4030`); a stage inserted before the cursor is history-only until an edge or a re-run reaches it | required |
| `remove-stage` | `stageId` | any state except `closed`; the stage has no attempts | n/a; a stage with attempts is routed around with `set-edge` | required |
| `reorder-stage` | `stageId`, `toIndex` or `stageIds` | any state except `closed`; only stages without attempts move | n/a | required |
| `set-edge` | `stageId`, `edge`, `to`, `maxRounds?` | any state except `closed` (today refused as terminal at `engine.ts:4092`), ran or not, traversed or not | the running attempt keeps its `activatedBy`; the new edge applies at its next verdict | required |
| `rerun-stage` (replaces `retry-stage`) | `stageId`, `from: "worktree" \| "last-passed" \| { checkpoint }`, `note?`, `discardWorktree?`, `stopCurrent?: boolean` | `needs_decision`, `completed` (reopens the record), or `running` | refused while any attempt on any stage is unsettled (`pending`, `spawning`, `running`, `reviewing`, `committing`, or `waiting` set), naming it, unless `stopCurrent: true`: then, inside the same transaction, that attempt is settled `failed` with a detail naming the mutation's `seq` and actor, the cursor moves to `stageId` and its `pending` attempt is created; after the transaction the host is stopped through `engine.ts:614`, with the terminal reaper (`engine.ts:3815`) as the backstop for a stop that fails. Effect `restarted-attempt`, the same as `edit-stage { restart: true }`, which is this stop for the same stage | `expectedRevision` required |
| `checkpoint` | `name` | any non-terminal state, no unsettled attempt on any stage | the controller commits the worktree (`git add -A` for read-write, declared outputs for read-only, `git.ts:123-141`) and records `{name, sha}` | required |
| `note` | `stageId`, `text` | any state except `closed`; at most `MAX_PENDING_STAGE_NOTES` pending on the stage | appended to `stage.notes`; the stage's next attempt to bind takes it (§3.2) and the response names that attempt in `appliesFromAttempt` | required |
| `answer` | `text` | the cursor stage is parked in `needs_decision`, no unsettled attempt on any stage | n/a: a parked attempt is settled and its host is not held; `answer` spawns attempt n+1 from the current worktree with the question and the text as a note (§3.6) | required |
| `pause`, `resume` | none | as today | sets or clears the `paused` flag; parked stays parked | optional (stop-and-look, see above) |
| `skip-stage` | none | `needs_decision`, no unsettled attempt on any stage | advances along the pass edge without resetting the worktree; the next stage's `pending` attempt is created in the same transaction | required |
| `start`, `update-draft`, `set-position`, `link-task`, `unlink-task`, `set-src`, `delete`, `close` | as today | as today | unchanged | optional |

Round limits: `set-edge` with `maxRounds` on a fail edge raises or lowers the budget at any time; the derivation stays `failEdgeRoundsUsed` (`engine.ts:1684-1689`), so lowering below the used count parks on the next fail verdict with the same detail as today. Swapping the next reviewer is `edit-stage` on the reviewer stage between rounds; nothing separate is needed. After completion the order is edit first, re-run second: the MCP binding requests a controller tick the moment `rerun-stage` returns (`bindings.ts:988`), and that tick binds the new attempt (§3.2), so an `edit-stage` sent after the re-run is `pending-next-attempt` for the attempt after. Admitting the edit on `completed` is what makes the ordering possible without a second payload on `rerun-stage`.
...
2. Otherwise the attempt is `waiting` when any of these holds, and no recovery check is spent while it does:
   ...
   - **Provider throttle.** The evidence is the record that closed the turn, which `terminalProviderMessageFromRecords` already reads into `terminalProviderMessage` (`durableEvidence.ts:118-148`), gated on the terminal turn reading (`durableEvidence.ts:176-178`). On Claude that is the assistant record flagged `isApiErrorMessage: true` whose `error` is in `TERMINAL_API_ERRORS` (`turnState.ts:18-24`, read at `turnState.ts:204`); the set is `rate_limit` and `authentication_failed`, and the CLI keeps every other API-error code inside an open turn because it retries those itself (#516), so no other code reaches settlement. On Codex it is the turn-end record carrying a failure (`codexTurnEndFailure`, `durableEvidence.ts:52-61`). The notice is classified a throttle by a new sibling of `usageLimit` on `terminalProviderMessage`, set when the record is a Claude `rate_limit` record or a Codex failure that `isCodexUsageLimit` (`durableEvidence.ts:68-71`) does not claim, and its text matches one of the throttle classes: `overloaded`, `rate limit`, `too many requests`, `429`, `529`, `stream disconnected`, or a 5xx status. The classes are one exported constant beside `TERMINAL_API_ERRORS` with a fixture per class, so a new provider wording is a one-line change. Two records are never throttles: a structured usage limit (`terminalProviderMessage.usageLimit`, `engine.ts:2154-2160`) and a Claude `rate_limit` record whose text names a usage window (`usage limit`, `limit reached`, `resets at`) are the account-limit case below; an `authentication_failed` record parks at once as today. `until` = the reset the notice states plus one minute when it states one (a `retry-after`, or the Codex `rate_limits` window `codexUsageLimitResetAt` reads, `durableEvidence.ts:79-106`), otherwise `since` + 10 min. The ceiling is 60 min of cumulative throttle waiting per attempt across at most three continuations, both declared in `limits.ts` beside the record bounds and counted in `waiting.continuations`. When a wait lapses the controller delivers one continuation (§3.6) into the same conversation; a dead host means attempt n+1 from the worktree; a lapsed ceiling parks the attempt naming the notice through `providerNoticeSummary` (`engine.ts:3716`), as today's park detail does.
...
`paused` is a flag beside the state. Pausing a parked record leaves `needs_decision` visible; resuming it leaves it parked with its decision affordance. The tick's skip condition (`engine.ts:3078`) and the terminal reaper's state gate (`engine.ts:2964`) read the flag. This removes `pausedState` and the masking in #852 from the durable record: from schema 6 the store never persists `state: "paused"`.

The readers outside the engine move later, and until they do the wire keeps its shape. Every transport and the liveness projector read records through one function, `getPipelines()` (`engine.ts`, called at `src/app/api/pipelines/route.ts:28`, `src/lib/mcp/bindings.ts:80` and `src/lib/lifecycle/liveness.ts:300`; the `list_pipelines` rows at `listProjection.ts:227-228` project from the same read). From slice 5 until slice 12 that function derives the legacy pair from the flag, `state: "paused"` and `pausedState` = the record's real state, so every current reader of `state === "paused"` keeps its contract unchanged: `src/lib/monitor/evidence.ts:78`, `src/components/tasks/taskReadiness.ts:103`, `src/components/pipelines/pipelineModel.ts:134,145,210`, `PipelineHub.tsx:102,132`, `PipelineStrip.tsx:285,759`, `StagePlaceholderPane.tsx:233`, `PipelineEditor.tsx:93`, `src/components/projectModel.ts:234`, `src/components/scheme/layout.ts:1025`, `offscreenClusters.ts:366` and `agentLinks.ts:381,483,523`. #852's persisted-state acceptance ("after pause, persisted state exposes the underlying `needs_decision` condition") is met by the record in slice 5; its affordance acceptance (the decision action visible while paused) arrives with slice 12, which drops the derivation and moves every reader above to the flag in one change (§6.3).

**A parked attempt is settled and its host is not held.** A `needs_decision` verdict is a verdict, so the attempt reads as turn-settled (`engine.ts:3815`) and the terminal host reaper stops its host on the next sweep (`engine.ts:2963-3056`), exactly as today. Holding that host until the operator answers was considered and cut: an idle structured host costs on the order of half a gigabyte of resident memory (#747), a decision routinely arrives hours later, and any affordable hold would lapse before most answers and leave two code paths chosen by timing. `answer { text }` on a parked cursor stage therefore has one behaviour: it creates attempt n+1, which binds (§3.2) and spawns from the current worktree, with no reset, under a titled note that carries the parked attempt's question (its verdict findings and `stateDetail`) and the operator's text. If the reaper has not yet reached the old host, the spawn names it in `supersedes` (`engine.ts:1976`) and the reaper stops it on its sweep; nothing ever delivers into a parked conversation. #852's contract holds: the park stays visible under pause, plain resume leaves it parked, one canonical action creates one fresh attempt carrying the answer, a repeated call with the same `clientRequestId` replays the same mutation, a call with a stale `expectedRevision` is refused with the mutations it missed, and a live host is never taken over.
...
- "нету функции изменения пайплайна … каждый раз пересоздавать" → §3.3: every definition and graph field of a started record is editable, on a completed record too; `rerun-stage` then reopens it; nothing requires recreating.
...
| #852 | yes | `paused` is a flag so the park stays visible and plain resume leaves it parked; `answer` creates one fresh attempt from the worktree carrying the question and the operator's text, idempotent on `clientRequestId` and fenced on `expectedRevision`; nothing is delivered into the parked host, which the reaper stops as today |
...
| Successor pipelines | yes | `set-edge` raises `maxRounds` after exhaustion; `add-stage` after start; `edit-stage` then `rerun-stage` on a completed record |
...
| 4 | feat(pipelines): edit-stage after start; attempts bind their definition at the spawning transition | `types.ts` staged constant and attempt fields (A); `engine.ts` patchPipeline, newAttempt and the spawning transition, `prompts.ts` renders from the attempt (B) | 1, 3 | `edit-stage` in `PIPELINE_ACTIONS_STAGED`, unreachable from both transports; admitted on `completed` as well as every non-closed state; binding is behaviour-preserving, the copy is taken at the instant the prompt is rendered today |
| 5 | feat(pipelines): rerun-stage from worktree or checkpoint with an explicit stop, checkpoint action, pause as a flag, skip without reset (#852) | `types.ts` (A); `engine.ts` including `getPipelines()` deriving the legacy pause pair, `git.ts` (B) | 4 | `rerun-stage` (with `stopCurrent`) and `checkpoint` staged; the `paused` flag is live on merge with `getPipelines()` deriving `state: "paused"` and `pausedState` for every reader outside the engine until slice 12 (§3.6), and the reset-free `skip-stage` is live on merge: together the #852 half that needs no new action; `retry-stage` keeps resetting until slice 10 aliases it |
| 6 | feat(pipelines): add, remove, reorder stages and rewire edges after start; editable round budgets | `store.ts` graph validators, `validation.ts` (A); `engine.ts` (B) | 5 | admitted after start, `completed` included, only with `expectedRevision`; a request without it meets today's refusal |
...
| 12 | feat(ui): after-start stage editor on the same PATCH payloads; readers move to the `paused` flag | `src/components/pipelines/**`, `taskReadiness.ts`, `evidence.ts` (G); `projectModel.ts`, `scheme/layout.ts`, `offscreenClusters.ts`, `agentLinks.ts` pause readers (E); `engine.ts` drops the derived pause pair from `getPipelines()`, `listProjection.ts` drops `pausedState` (B) | 10, 11 | deferred; named here so nobody designs a second API for it. The derivation from §3.6 is removed in this slice, so every `state === "paused"` reader listed there moves in the same change, lanes G and E landing before B drops the pair |

Lane B lands slices 2 to 8 in order, then its parts of 9a and 9b. Lane A lands its parts of 1, 3, 4, 5, 6, 7, 9b and 10 in that order. Lane C lands 9a's descriptions, 9b's deletions and 10. Lanes D and F each own one slice's worth of files; E and G each land in two (9b and 12 for E; 9b's card branch and 12 for G). Slice 2 goes first after the log because it removes the most expensive failure class in §1.5.
...
- **Re-running from an older pass's head**: every pass records `headAtSettle` on its attempt (§3.5) and the latest on `lastPassedCommit`, so a `from` variant naming a past attempt could resolve from the record. The quote asks for the current worktree and named checkpoints; automatic per-pass checkpoints were cut in revision 4 as a second copy of those fields, and no such variant is added until a lane asks.
