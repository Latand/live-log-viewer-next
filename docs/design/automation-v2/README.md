...
| `rerun-stage` (replaces `retry-stage`) | `stageId`, `from: "worktree" \| "last-passed" \| { checkpoint }`, `note?`, `discardWorktree?` | `needs_decision`, `completed` (reopens the record), or `running` when no attempt on any stage is unsettled | refused while any attempt on any stage is unsettled (`pending`, `spawning`, `running`, `reviewing`, `committing`, or `waiting` set), naming it; stop the running attempt with `edit-stage { restart: true }` first | `expectedRevision` required |
...
| `pause`, `resume` | none | as today | sets or clears the `paused` flag; parked stays parked | optional |
