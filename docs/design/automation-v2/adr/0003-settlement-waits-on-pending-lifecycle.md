...
2. Otherwise the attempt is `waiting` and no recovery check is spent while one of these holds:
   ...
   - **provider throttle**: the record that closed the turn (`terminalProviderMessageFromRecords`, `src/lib/pipelines/durableEvidence.ts:118-148`; on Claude the `rate_limit` API-error record, `turnState.ts:18-24`) matches a throttle text class held in one exported constant; a usage limit is the next case. Wait until the stated reset plus one minute, or 10 min without one (#611 shape), then deliver one continuation into the same conversation; a dead host means a fresh attempt from the worktree. Ceiling: 60 min per attempt over at most three continuations (README §3.4).
