...
9. **No confirmation prompts; receipts carry the inverse.** Kill, close, rotate, retry, skip, switch account, use a reset: each acts on the tap that names it and answers with a receipt on the dock's top edge, for 4 s, with the inverse action as a 44 px text button when one exists (Kill → Respawn, Close → Reopen, Archive → Restore, Skip → Retry stage, switch → Switch back).
...
Screens: Board, Conversation, Pipelines, Pipeline, Accounts & limits. Sheets: Projects, Needs you, board `⋯`, Host, Search, Seat, Rotate draft (fullscreen), conversation `⋯`, Switcher, Next message. Sheets take at most 88% of the height, keep the screen behind visible and dimmed, and are modal in the sense `useModalLayer` already implements.
...
- Rows are dot · title · one meta line · one trailing element. On a row that needs the operator the edge and the badge are the two coloured elements; the meta reads `waiting 9 min · ⌘ gpt-5.6`, in secondary text, and the title may take two lines. On a working row the meta reads `working 12:40 · <running tool or last line> · ✦ Opus`. The state phrase never truncates.
...
- **State precedence**, computed once in `stateBits` and used by every surface:
  ...
  | killed | danger | `killed · messages queue` · › (Recent) | same, danger | placeholder `killed · text queues until a respawn`; the slot is **Respawn** |
  | stalled | danger edge | `14 min · ✦ Opus` · badge `stalled` (Needs you) | `stalled · 14 min` | send; `⋯ › Kill agent` hints "stalled" |
  | limit | warning edge | `Main resets 16:40 · ✦ Opus` · badge `limit` (Needs you) | `limit · Main resets 16:40` | the chip reads `Opus · Main at limit` in warning and its sheet offers the other account inline |
  | held | warning | `held · 2 messages queue` · › (Working) | same, warning | placeholder `held · text you send queues` |
...
- **Arrival** (`chat-arrival`): a decision that arrives while the operator reads another conversation shows in the banner slot (`Needs you · plan approval / Migrate accounts to the new binding`, body tap opens it and stamps seen, `×` dismisses) and collapses into the badge after ~6 s. The prototype runs the timer; lane 8 tests it.
...
- **Receipts**: raised surface, success edge, on the dock's top edge (in flow inside a sheet), 4 s, with the inverse action as an accent text button. The scroller under a receipt gets padding so no control sits beneath it.
...
| **P1-3** only the happy path | Six states as fixture scenarios and screens: `board-noseat` (invitation card, create draft), `board-degraded` (info banner, host badge), `chat-offline` (banner, bar meta `offline · reconnecting`, Queue slot, "Held until reconnected"), `chat-held`, `chat-limit` (chip in warning, alternative account in the chip's sheet), `chat-stalled` (danger edge, Kill hint). One precedence list in §4.2, implemented in `stateBits`. | `fixture.js` `SCENARIOS`, `app.js` `stateBits`, `banner`, `composer`, `modelSheet` |
...
| **P2-6** receipts cover controls, no undo | Receipts sit on the dock's measured top edge (in flow above a sheet's footer), 4 s, and the scroller under them gets padding so no control is beneath; the gate checks a receipt against every control. Inverse actions: Kill → Respawn, Close → Reopen, Archive → Restore (pipeline and project), Skip → Retry stage, switch account → Switch back; Use one reset states the new window. The flow kills c1 from `⋯`, checks the receipt covers nothing, and respawns from it. | `app.js` `toast`, `toastHtml`, `render`; `styles.css` `.toast` |
