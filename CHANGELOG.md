...
- The documented release path works as written (#1309). The deploy protocol
  ...
  `git pull --ff-only` before the release, and the pull was wrong in its own
  ...
  to and validated `origin/main`, a value `POST /api/runtime/deployments` has
  never accepted for `revision`; it now takes a full lowercase commit SHA or no
  argument at all, and the no-argument default resolves the canonical
  `refs/heads/main` tip with `git ls-remote`, which fetches nothing into the
  checkout and moves no ref in it. The refusal also used to arrive underneath
  `deployment key: …`, so a request that deployed nothing read like a deployment
  that had started: nothing that reads as a started deployment is printed now
  until the endpoint has accepted the request, and a refused request prints its
