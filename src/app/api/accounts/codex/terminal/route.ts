...
export async function POST(req: NextRequest) {
  ...
  try {
    return NextResponse.json({ ok: true, ...resolveAccountTerminalCommand("codex", body.id) });
  } catch (error) {
    if (error instanceof TerminalAccountUnavailableError) return NextResponse.json({ error: error.message }, { status: error.status });
    console.error("[accounts] codex terminal command failed:", error);
    const detail = error instanceof Error ? error.message : String(error);
    return NextResponse.json({ error: "Codex terminal command failed", detail }, { status: 500 });
  }
}
