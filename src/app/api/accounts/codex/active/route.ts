...
export async function POST(req: NextRequest) {
  ...
  let body: { id?: unknown; mode?: unknown; requestId?: unknown; previewRevision?: unknown };
  ...
  try {
    ...
    if (body.mode === "migrate") {
      ...
      const result = await createMigrationIntent("codex", body.id, "manual", requestId, body.previewRevision as number);
