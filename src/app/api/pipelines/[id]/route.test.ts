...
mock.module("@/lib/pipelines/engine", () => ({
  ...
  tickPipelines: async () => ({ pipelines: [pipeline], changed: false }),
...
test("pipeline PATCH accepts control and task-link actions", async () => {
  ...
  for (const action of ["start", "update-draft", "add-stage", "remove-stage", "reorder-stage", "set-edge", "pause", "resume", "retry-stage", "skip-stage", "override-stage", "link-task", "unlink-task", "set-src", "delete", "close"]) {
