/**
 * Batch host-target lookup for the conversations on screen: one request answers
 * "who currently hosts this transcript?" for up to 64 of them, so the board does
 * not fan out a request per card.
 *
 * Mounted at `/api/conversation-host/targets` and at the legacy
 * `/api/tmux/targets`. NO TMUX IS INVOLVED in resolving a structured host: the
 * answer comes from the transcript-host snapshot, and the display string it
 * returns names whatever host owns the conversation (#1301).
 */
...
export async function conversationHostTargetsPOST(req: NextRequest): Promise<NextResponse<TargetBatchResponse | { error: string }>> {
