...
export async function POST(req: NextRequest): Promise<NextResponse<Record<string, unknown> | ApiError>> {
  ...
  const result = await executeRealtimeControl(body, undefined, {
    ...
    /* Opening and closing the call is the operator's own act, and on this
       loopback single-user app the same-origin browser IS the operator (the
       cross-origin rejection above is the perimeter). The only caller refused
       transport is an AGENT, which names itself by presenting its capability —
       the distinction this guard was always about. Designation and every other
       operator-only action keep the stricter possession check. */
