...
export function ProjectRail({ files, selected, onSelect }: Props) {
    ...
    <aside className="flex w-[248px] shrink-0 flex-col border-r border-line bg-panel">
      ...
      <div className="px-2.5 pb-1 pt-2.5">
        <input
          ...
          placeholder="Фільтр проєктів…"
          value={query}
          onChange={(event) => setQuery(event.target.value)}
        />
      </div>
      <nav className="flex-1 overflow-y-auto px-2 pb-3 pt-1" aria-label="Проєкти">
        <RailRow
          label="Огляд"
          live={0}
          attention={0}
