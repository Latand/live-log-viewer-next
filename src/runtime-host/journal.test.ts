...
test("terminal image delivery retry on a replacement host mints one fresh operation", () => {
  ...
  const images: StructuredImageRef[] = [
    { sha256: "f".repeat(64), mime: "image/webp", bytes: 80 },
  ];
  const contentDigest = structuredContentDigest({ text, images });
  journal.executeOperation({
    kind: "send",
