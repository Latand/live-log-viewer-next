...
test("readLegacyPrefs reconstructs the three old localStorage tiers", () => {
  ...
  expect(readLegacyPrefs("proj", storage)).toEqual({ manual: ["/a"], hidden: ["/b"], expanded: ["/c"], favorites: [], foldedEngineChildIds: [], expandedEngineTrayParentIds: [], idleCollapseMinutes: 120, viewMode: "list", taskPanelOpen: true });
  expect(readLegacyPrefs("other", storage)).toEqual({ manual: [], hidden: [], expanded: [], favorites: [], foldedEngineChildIds: [], expandedEngineTrayParentIds: [], idleCollapseMinutes: 120, viewMode: null, taskPanelOpen: true });
