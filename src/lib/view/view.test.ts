...
describe("selection made OUTSIDE scheme mode reaches the snapshot (#771)", () => {
  /* The publication half of #771 lives in the client (three publishers reporting
     one lifted set). This is the consumer half, asserted end to end: given a view
     session whose mode is `list` or `mobile-focus` and whose selectedPaths are
     non-empty, the snapshot must return those conversations AND their content.
     `scopedPaths` and `validateExplicitMembership` already read `selectedPaths`
     without caring which mode reported them, so nothing in snapshot.ts changed —
     this pins that down so a future edit cannot quietly make `selected` a
     scheme-only scope. */
