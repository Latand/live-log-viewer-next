...
test("rotation composes a bounded handoff, switches designation atomically, and links both cards", async () => {
  ...
  expect(spawnedPrompt).toContain(`conversation_messages({"clientRequestId":"rotation-predecessor-recent-turns","conversationId":"${NEW_ID}","roles":["user","assistant"],"limit":40})`);
